...
check_return_code "git checkout $feature_name"
...
check_return_code "git rebase -i $(git merge-base $feature_name dev)"

Write-Output 'git fetch origin dev'
Start-Sleep -s 1
git fetch origin dev
check_return_code "git fetch origin dev"

Write-Output 'git rebase -i dev'
Start-Sleep -s 1
git rebase -i dev
check_return_code "git rebase -i dev"

Write-Output 'git push origin --force-with-lease'
Start-Sleep -s 1
git push origin --force-with-lease
check_return_code "git push origin --force-with-lease"

Write-Output "git checkout dev"
Start-Sleep -s 1
git checkout dev
check_return_code "git checkout dev"

Write-Output "git merge $feature_name"
Start-Sleep -s 1
git merge $feature_name
check_return_code "git merge $feature_name"

Write-Output "git push"
Start-Sleep -s 1
git push
check_return_code "git push"
